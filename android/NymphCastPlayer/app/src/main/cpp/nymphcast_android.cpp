...
JNIEXPORT jstring JNICALL Java_com_nyanko_nymphcastplayer_NymphCast_sendApplicationMessage
	...
	std::string apppIdStr = convValue;
	env->ReleaseStringUTFChars(appId, convValue);
	
	convValue = env->GetStringUTFChars(id, &isCopy);
...
JNIEXPORT void JNICALL Java_com_nyanko_nymphcastplayer_NymphCast_findServers
	...
	for (int i = 0; i < remotes.size(); ++i) {
		env->SetObjectArrayElement(remoteArray, i, env->NewStringUTF(remotes[i].ipv4));
	}
...
JNIEXPORT jboolean JNICALL Java_com_nyanko_nymphcastplayer_NymphCast_connectServer
	...
	remotesMutex.lock();
	bool retval = client.connectServer(remotes[index].ipv4, activeHandle);
	remotesMutex.unlock();
...
JNIEXPORT jboolean JNICALL Java_com_nyanko_nymphcastplayer_NymphCast_castFile
	...
	// We receive a file descriptor here that we can pass straight to the library.
	client.castFile(activeHandle, fd);
