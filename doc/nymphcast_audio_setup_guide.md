# NymphCast Audio Setup Guide #

NymphCast Audio (NC-A) is a configuration of the NymphCast receiver that supports the playback of audio content, while disabling any video output. This document covers the steps required to set up a NymphCast Audio receiver system.

## Features of NymphCast Audio ##

When the receiver is configured in NC-A mode, the following features are supported:

- Playback of all media content (with only audio output).
- Control with NymphCast clients (including the Player).
- NymphCast Apps.

The audio quality and other details are dependent on the underlying hardware.

## Requirements ##

In order to set up an NC-A system, the following hardware and software requirements have to be met:

- System running a supported operating system (Linux, Windows, others are experimental).
- Audio output configured in the OS.
...
During the installation, the question is asked which configuration of NC to use. Choose `audio` here to get the NC-A configuration.
